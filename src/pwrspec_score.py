...
def pwrspec_score(
    ...
    z_true = z.clone().to(device)

    score = torch.zeros_like(x_new)
    k = 0
    while k < x_new.shape[-1]/2 or (k == x_new.shape[-1]//2 and x_new.shape[-1]%2 == 0):
        if CLT:
            tmp = pwrspec_CLT_score_comps(x_new, x_tilde, z_true, M, k, device=device)
        else:
            tmp = pwrspec_score_comps(x_new, x_tilde, z_true, M, k, device=device)
        score += tmp
        k += 1
