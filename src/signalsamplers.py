...
from pwrspec_density import pwrspec_density
...
class HatSampler(SignalSampler):
    ...
    def condition_distr(
        ...
        if self.distr is None:
            masses = torch.zeros((self.length,), device=self.device)
            hat_pwrspecs = fft(self.hats, norm='ortho').abs().square()
            for hat_idx in range(self.length):
                if use_CLT:
                    ...
                    scaled_pwrspec = num_samples * sample_pwrspec / (sigma ** 2)
                    terms = np.zeros(((self.length+1)//2,))
                    for k in range((self.length+1)//2):
                        if k == 0 or (k == self.length // 2 and self.length % 2 == 0):
                            k_fac = 1
                        else:
                            k_fac = 2
                        terms[k] += ncx2.logpdf(
                            x=k_fac*scaled_pwrspec[k].numpy(force=True), 
                            df=k_fac*num_samples,
                            nc=k_fac*hat_pwrspecs[hat_idx, k].numpy(force=True),
                            # scale=(sigma**2)/(k_fac*num_samples),
                        )
                    masses[hat_idx] += np.exp(terms.sum(axis=-1))
                else:
                    masses[hat_idx] += pwrspec_density(
                        rho=sample_pwrspec,
                        rho_x=hat_pwrspecs[hat_idx, :],
                        sigma=sigma,
                        num_samples=num_samples,
                        device=self.device,
                    )
            self.distr = torch.cumsum(masses / masses.sum(), dim=-1)
        else: 
            raise NotImplementedError("Sampler can currently only be conditioned once.")
    
    def sample_hat_distr(self, num):
        if self.distr is None:
            idxs = torch.randint(
                low=0, 
                high=self.length, 
            ...
            idxs = torch.argmin((rands[:, None] > self.distr[None, :]).long(), dim=1)
        return idxs
    ...
    def __call__(
        ...
        shifts = self.generate_shifts(num, do_random_shifts)
        hat_idxs = self.sample_hat_distr(num)
        signals = self.signal + self.hats[hat_idxs, :]
        signals_circulants = circulant(signals, dim=-1)
        samples = signals_circulants[torch.arange(0, num), shifts, :]
        return samples
